                ...
                if (passengerCurrentLocation != null) {
                    ...
                    requestCar.saveInBackground(new SaveCallback() {
                        ...
                        public void done(ParseException e) {
                            if (e == null) {
                                Toast.makeText(PassengerActivity.this, "Car Request is send", Toast.LENGTH_SHORT).show();
                                btnBookCar.setText("Cancel the Uber order");
                    ...
                    Toast.makeText(this, "Unknown Error. Please turn ON location", Toast.LENGTH_LONG).show();
                }
            }
        } else {
            // query
            ParseQuery<ParseObject> carRequests = ParseQuery.getQuery("RequestCar");
            carRequests.whereEqualTo("username", ParseUser.getCurrentUser().getUsername());
            carRequests.findInBackground(new FindCallback<ParseObject>() {
                @Override
                public void done(List<ParseObject> requestList, ParseException e) {
                    if (requestList.size() > 0 && e == null) {
                        isRequestCancelled = true;
                        btnBookCar.setText("Request a new uber");
